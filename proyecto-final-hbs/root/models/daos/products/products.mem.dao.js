...
class ProductsMemDao {
    ...
    async edit(object){
        ...
        try{
            let contenido = await fs.promises.readFile(`./db/memory/${this.name}`, 'utf-8');
            let contenidojson = JSON.parse(contenido);
            const productIndex = contenidojson.findIndex((product) => product.id === object.id);
            console.log(productIndex)
            contenidojson[productIndex] = object;
            await fs.promises.writeFile(`./db/memory/${this.name}`, JSON.stringify(contenidojson))
        } 
        catch(error){
            console.log(error)
        }
